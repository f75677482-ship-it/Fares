...
// --- الإعدادات ---
const token = '8631941557:AAHJ_97NplwcLMkee0-Zrf2FY5XqmI6E_0I'; // توكن تليجرام الخاص بك
...
async function startBot(chatId, phone) {
    ...
    const sock = makeWASocket({
        ...
        // تعريف المتصفح ضروري لتجنب رفض طلب الكود
    ...
    if (!sock.authState.creds.registered) {
        await delay(5000); // تأخير لضمان استقرار الاتصال قبل الطلب
        try {
            const code = await sock.requestPairingCode(phone);
            return code;
        } catch (e) {
            console.error("Error in Pairing:", e);
            return null;
        }
    }
...
app.post('/api/pairing', async (req, res) => {
    ...
    if (code) {
        ...
        res.json({ success: false, error: "تأكد من الرقم أو انتظر دقيقة" });
    }
});
...
bot.on('message', async (msg) => {
    ...
    if (msg.text && /^[0-9]{10,}$/.test(msg.text.replace('+', ''))) {
        ...
        const cleanNumber = msg.text.replace(/[^0-9]/g, '');
        const code = await startBot(chatId, cleanNumber);
        if (code) {
